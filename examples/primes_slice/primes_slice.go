...
func main() {
	...
	// find primes_slice in first 1000 numbers
	numbersToCheck := make([]uint, 100_000)
	...
	start := time.Now()
	// find primes_slice via pool
	// if callback job is done, result is sent to result channel, and I just iterate over
	for result := range simpleWorkerPool.DoWorkFromSlice[uint, uint](context.Background(), numbersToCheck, func(ctx context.Context, number uint) simpleWorkerPool.JobResult[uint] {
